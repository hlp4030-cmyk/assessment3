...
function getSessionAccessTokenOrThrow(): string {
  const sessionRaw = localStorage.getItem('eatup-session')
  const session = sessionRaw ? (JSON.parse(sessionRaw) as { accessToken?: string }) : null
  const token = session?.accessToken?.trim()
  if (!token) {
    throw new Error('Authentication required. Please log in again to access ingredient and recipe data.')
  }
  return token
}
...
// New helpers to support remote Supabase integration

export type RemoteIngredient = {
  id: string
  name?: string
  category?: string
...
const FALLBACK_IMAGE_URL = 'https://images.unsplash.com/photo-1542838132-92c53300491e?auto=format&fit=crop&w=800&q=80'

function normaliseIngredientRow(row: any): RemoteIngredient {
  const resolvedId = String(row?.id ?? `unknown-${Math.random().toString(36).slice(2, 8)}`)
  return {
    id: resolvedId,
    name: row?.name ?? 'Unknown ingredient',
    category: row?.category ?? 'General',
    standard_unit: row?.standard_unit ?? 'ea',
    default_shelf_life_days: Number(row?.default_shelf_life_days ?? 7),
    default_quantity: Number(row?.default_quantity ?? 0),
    image_url: row?.image_url ?? FALLBACK_IMAGE_URL,
    expiration_date: row?.expiration_date ?? todayIso(),
    meal_type: row?.meal_type ?? 'general',
  }
}
...
export type RemoteRecipe = {
  ...
  image_url: string
  required_ingredients: { id: string; qty: number; unit: string }[]
  instructions: string
}

export async function fetchRecipes(): Promise<RemoteRecipe[]> {
  const accessToken = getSessionAccessTokenOrThrow()
  ...
  const data = (await response.json()) as RemoteRecipe[]
  if (!response.ok) {
    const msg = data?.length ? JSON.stringify(data) : 'Failed to fetch recipes'
    throw new Error(msg)
  }
  return data
}

export async function fetchIngredientsByIds(ids: string[]): Promise<Record<string, RemoteIngredient>> {
  const accessToken = getSessionAccessTokenOrThrow()
  const resp = await fetch(`${BACKEND_BASE_URL}/ingredients`, {
    method: 'GET',
    headers: {
      Authorization: `Bearer ${accessToken}`,
    },
  ...
  if (!resp.ok) {
    throw new Error('Failed to fetch ingredient records by id')
  }
  const map: Record<string, RemoteIngredient> = {}
  for (const raw of rows) {
    const r = normaliseIngredientRow(raw)
    const key = r.id
    if (!ids || ids.length === 0) {
      if (key) map[String(key)] = r
      continue
    }
    if (key && ids.includes(String(key))) map[String(key)] = r
  }
  return map
}

export async function fetchIngredientsMaster(name?: string): Promise<RemoteIngredient[]> {
  const accessToken = getSessionAccessTokenOrThrow()
  const query = name && name.trim().length > 0 ? `?name=${encodeURIComponent(name.trim())}` : ''
  const response = await fetch(`${BACKEND_BASE_URL}/ingredients${query}`, {
    method: 'GET',
    headers: {
      Authorization: `Bearer ${accessToken}`,
    },
  ...
  if (!response.ok) {
    throw new Error('Failed to fetch ingredients master list')
  }
  return (Array.isArray(data) ? data : []).map((r) => normaliseIngredientRow(r))
}

export async function fetchInventoryIngredientIds(accessToken: string): Promise<string[]> {
  // Try to fetch fridge items for the current user, map to ingredient ids
  const rows = await getMyFridgeItems(accessToken)
  const ids = rows.map((r) => r.ingredient ?? '')
